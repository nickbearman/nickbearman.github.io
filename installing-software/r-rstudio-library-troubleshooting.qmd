...
![](images/r-install-packages-sf-success.png)

- In some versions of R, the text may be in Red - do not worry, red text does not always mean an error!
...
![](images/r-install-packages-tmap-success.png)

- Then load the library by running `library(tmap)` *it may take 5-10 seconds to load before the `>` on the next line appears*. 
